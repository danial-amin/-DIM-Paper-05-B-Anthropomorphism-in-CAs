...
\acmConference[UMAP '26]{Make sure to enter the correct
...
%% article.
\begin{abstract}
Conversational agents (CAs) increasingly serve as anthropomorphic interfaces to computing systems. Research shows it is important that when users engage with CAs, the CAs exhibit humanlike features---but what features and how should these features be implemented? This meta-analysis investigates how CAs are anthropomorphized in IS research. We identify commonly used anthropomorphic attributes, strategies for implementing anthropomorphism, and outcome considerations. We analyzed 79 articles that develop CAs using anthropomorphism (2018-2025) from four databases, systematically coding 30 variables covering CA's anthropomorphic attributes, implementation methods, and user outcomes for anthropomorphic CAs. The analysis shows that...
\end{abstract}
...
Conversational agents (CAs) (a.k.a., chatbots, virtual agents, virtual assistants, virtual companions) are software systems designed to interact with users through natural language, typically using text or voice interfaces \citep{diederich_conversational_2019}. Examples include Alexa (Amazon), Anna (IKEA), ChatGPT (OpenAI), Cortana (Microsoft), and Siri (Apple). These CAs utilize natural language processing (NLP), machine learning (ML), or LLMs to comprehend user inputs and provide tailored responses. CAs now serve as instruments of user modeling and personalization (UMAP) for customer service, healthcare, education, and social companionship \citep{araujo2018living,brandtzaeg2017people}. 

Unlike traditional user interfaces (UIs) that require structured navigation, these systems allow natural language interaction, influencing how users access information and services. The design of these CAs, particularly whether to assign human-like characteristics--that is, \textit{anthropomorphism}--represents a critical decision affecting user acceptance and system effectiveness \citep{pfeuffer2019anthropomorphic,ashfaq2020deploying}, as well as user engagement \citep{gong_how_2008}. As argued by \citet{chandra_be_2022}, ``the business impact of conversational AI is contingent on customers using and adequately engaging with these tools'' (p. 969). In a similar vein, \citet{gong_how_2008} argues that anthropomorphism, ``enabling computers with human-like characteristics and capabilities, has been a driving philosophy behind the advances of computer technology'' (p. 1494). 

%Information systems (IS) often involve repositories of knowledge that users access to obtain information in simple, easy, and usable ways \citep{boell2015information}. With the advancement of large language models (LLMs) \citep{ampel2025large}, conversational agents (CAs) powered by these models enable interaction between systems and users through natural language interaction \citep{teubner2023welcome,zhu2023large}. This interaction creates value in many business functions, such as customer service, marketing, sales \citep{diederich_conversational_2019}. 

Developments in large language models (LLMs) exemplify the topicality of anthropomorphism for user engagement: for example, OpenAI's introduction of ChatGPT 5.1, described as being ``warmer'' and ``having more personality,'' indicates that leading AI organizations are deliberately humanizing these systems \citep{hayden_field_openai_2025}. This humanization, referred to in research as anthropomorphism, is the central theme of the current work. %As a result of anthropomorphism, users often treat CAs as if they were real people, even when the CA openly discloses that it is non-human. Existing research suggests that many users are empathic to CAs ~\citep{bach2024systematic,de2020millennials}. Prior findings indicate that CA behavioral cues activate these responses in many people. \textit{Why does even minimal anthropomorphism have such a profound influence on people?} Understanding this motivational question requires an examination of how anthropomorphism influences human–AI interaction \citep{nass1997machines,lew2023social}, which we do in this research. 

Typically, anthropomorphism in CAs (see Figure \ref{fig:CAs}) involves assigning human characteristics such as names, personality traits, and conversational styles \citep{epley2007seeing}. %This practice draws from the \textit{Computers Are Social Actors (CASA)} theory, which demonstrates that people automatically apply social rules when interacting with systems that display human-like cues, responses, and behaviors \citep{nass1994computers,reeves1996media}. Understanding the psychology of anthropomorphism in human-AI interaction has become increasingly important due to the fact that CA systems proliferate in society at an unprecedented speed \citep{de2020psychology,sundar2020rise,peter_benefits_2025}. The progress in LLMs has made convincingly human-like conversation feasible \citep{pnas2025anthropomorphic}, raising urgent questions about optimal design of anthropomorphism in CAs.
...
\begin{figure*}[htbp]
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=0.5\textwidth]{Figures/non-human.jpg}
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=0.5\textwidth]{Figures/juve.jpg}
        \caption{Anthropomorphic CA with 3D avatar}
        \label{fig:ca_human}
    \end{subfigure}

    \caption{\textbf{Anthropomorphism in conversational agents.} Examples show (a) a non-anthropomorphic CA used to provide information to students, and (b) an anthropomorphic CA that includes a 3D avatar.}
    \label{fig:CAs}
\end{figure*}

%A specific form of anthropomorphism involves assigning explicit \textit{personas}, which are defined sets of attributes that affect the CA's identity and responses. The PersonaChat dataset illustrates this approach through profile descriptions with characteristics such as hobbies, occupations, and preferences \citep{zhang2018personalizing}. 

Research on CAs has expanded rapidly since the introduction of LLMs \citep{chen2024trends}, exploring effects on factors such as conversation fluency, user experience (UX), and interaction outcomes. While CAs have been extensively studied in human-computer interaction (HCI) and natural language process (NLP) fields.
Despite this growing interest, \textbf{the UMAP field lacks systematic understanding of which anthropomorphic attributes actually matter for user outcomes}. In 2019, Diederich et al. (\citeyear{diederich_conversational_2019}) reviewed 36 articles to investigate the state of CA research, identifying gaps concerning study topics, methods, and theories\footnote{In a follow-up study published in 2022, Diederich et al. (\citeyear{diederich_design_2022}) performed a more extensive review with 262 studies, but this focused on human-computer interaction research.}. Although their work serves as an important precursor to the study of CAs in IS, a refresher on this topic, given the impact of LLMs, is needed. Articles examine individual characteristics such as names \citep{go2019humanizing}, gender, and communication style \citep{seeger2021texting}, but findings remain scattered throughout contexts and research articles. Some research demonstrates that anthropomorphic features increase trust through social presence \citep{konya2023anthropomorphism,blut2021understanding}, while other articles identify negative effects when human-like characteristics create unrealistic expectations \citep{crolic2022blame,ciechanowski2019shades}.

This fragmentation creates practical problems for the CA field. Organizations deploying conversational UIs lack evidence-based guidance on design decisions. The ability of researchers to build systematically on prior work is hindered without a comprehensive synthesis of anthropomorphic CA attributes, implementation methods, and user outcomes.
%However, the design of anthropomorphic features in CAs directly impacts critical organizational outcomes. T
For example, trust formation represents a primary concern, as users must decide whether to accept AI-generated information \citep{meyer2023leverage,følstad2021chatbots,nordheim2019trust}. %Personalization and adaptability determine whether CAs can serve diverse user populations with varying expertise levels, cultural backgrounds, and interaction preferences \citep{huang2023personalized,chaves2021should}. 
The lack of research on how to design anthropomorphism in CAs, including whether to assign names, gender, personality traits, or adaptive behaviors, results in CA developers making design choices based on intuition or anecdotal information rather than validated research.

Against this backdrop, this meta-analysis addresses a key research question: \textbf{How do anthropomorphic attributes of CAs affect user outcomes?}

Understanding how anthropomorphic attributes affect user outcomes is essential for evidence-based CA design. Research reports effects on trust and acceptance \citep{kraus_role_2021,chandra_be_2022}, user experience and satisfaction \citep{lieb_student_2024,liu_older_2023}, behavioral patterns such as engagement and compliance \citep{mehra_chatbot_2021,chen_you_2021}, and task performance \citep{zheldibayeva_genai_2025,baihaqi_rapport-driven_2024}. However, findings often appear contradictory, with some studies showing positive effects of personality design while others find no significant effect. Our synthesis of RQ3 organizes these empirical findings into outcome categories and identifies conditions under which anthropomorphic design produces its effects.

% Concerning RQ3, in practical applications, CAs serve distinct purposes, from accomplishing specific tasks (informational, advisory, educational, professional) to providing social value (companionship, entertainment). These purposes may require different anthropomorphization approaches, as task-oriented interactions focus on achieving external goals while social-oriented interactions center on the relational experience itself. While the task-oriented CAs have been researched over time, social CAs are relatively new, and it would be interesting to understand whether the two types of CAs have different or similar anthropomorphic attributes.

This meta-analysis makes one main contribution. We synthesize empirical findings on user outcomes, organizing results into four outcome categories and identifying factors that explain variation across studies. As such, the findings are important to the IS research community, but they also matter for the business sector, because companies need information on which human-like traits to embed in CAs to enable beneficial user interactions.

 %, which for the purpose of this study is IS research community.

\section{Related Work}

% --- Section 2.1: Broad IS Context (Preserved your original text) ---
\subsection{The History of Conversational Agents as (Sort of) People}
CAs have existed for several decades, since Weizenbaum's ELIZA in 1966, which simulated a psychotherapist \citep{weizenbaum_elizacomputer_1966}\footnote{Before that, Turing had discussed the prospect of a machine imitating humans in conversations in his seminal article from 1950 \citep{turing_computing_1950}.}. Despite having been studied for decades, it is only the LLM technology that brought about a decisive breakthrough in the abilities of CAs to carry out conversations with human users. Peter et al. (\citeyear{peter_benefits_2025}) use the term ``anthropomorphic conversational agents'' to characterize recent LLMs that, despite lacking genuine social understanding, excel at simulating empathy and persuasion to a degree that often surpasses human capability and renders them nearly indistinguishable from real people. Now, even though CAs have been studied extensively in fields like HCI and NLP, investigating anthropomorphism or ``human-likeness'' as a means to improve user engagement with CAs deviates from most earlier IS user engagement research, which has largely centered on the functional or instrumental value of information technology \citep{chandra_be_2022}. For this reason, it is worthwhile to discuss the nature of CAs as forms of IS.
...
Furthermore, CAs can be categorized into (1) text-based CAs (written text only, no embodiment \footnote{Embodiment refers to the physical or visual representation of a CA, such as an avatar or robot body. While embodiment is one possible anthropomorphic attribute, anthropomorphism is a broader concept encompassing any human-like characteristic including name, personality, communication style, and behavior.}), (2) virtual CAs (written and spoken interaction with virtual interactive avatars), (3) speech-based CAs (spoken interaction without embodiment), and (4) physical CAs (spoken interaction with physical embodiment) \citep{diederich_design_2022}. Across these different types, a shared trait is the reliance on natural language for interaction \citep{diederich_design_2022}. 
Another key trait of CAs is that, unlike navigation-based UIs, CAs adapt to user needs \citep{allouch2021conversational,jain2018evaluating}. For example, a student can ask a CA to provide steps to solve a mathematics problem, and the CA would do so. To accommodate such interactions, IS design is evolving from \textit{navigation-based} formats to \textit{conversational interaction} \citep{finch2023dont,dam2024complete}. A CA can take multiple forms, including static or interactive virtual avatars, physical embodiments, or a disembodied form that lacks any visual representation and relies solely on a natural language (chat) UI \citep{diederich_conversational_2019,diederich_design_2022}. These different implementations shape how users perceive the CA \citep{peter_benefits_2025}, while making systems and information more accessible. There is even an argument that one of the core objectives of computer science is to ``to make computers more accessible and more natural to interact with, \textit{by making computing more human-like}'' (emphasis by us) \citep{peter_benefits_2025} (p. 1).
...
\subsection{Conceptual Underpinnings of Anthropomorphism}

Anthropomorphism, the attribution of human characteristics, emotions, or behaviors to non-human entities, draws on the CASA paradigm, which posits that users unconsciously apply social rules to technology \citep{nass1994computers}. Interacting with an IS through natural language can increase ease of use and enable faster completion of user requests, while providing a pleasant human-like interaction \citep{diederich_design_2022}. Anthropomorphism is a critical component in CAs, because ``[user] engagement [with CAs] depends, in turn, on conversational AI’s similarity, or likeness to the human beings it is intended to replace'' \citep{chandra_be_2022} (p. 969).

The theoretical foundation for understanding why users respond socially to CAs can be derived from the CASA paradigm \citep{nass2000researching}. Introduced by Nass et al. (\citeyear{nass1994computers}), CASA argues that humans apply social rules to computers even when they know they are interacting with machines. The key insight was that social responses to computers ``are automatic and
unconscious.'' (p. 77) \citep{nass1994computers}. Rather, these responses are automatic and easy to trigger. The underlying mechanism is \textit{mindlessness}---a psychological state where individuals rely on overlearned social scripts without conscious awareness \citep{nass2000machines}. When computers display minimal social cues such as language or voice, users automatically apply human-human interaction scripts. These cued behaviors include, for example, (1) gender stereotyping (e.g., rating female-voiced computers as more knowledgeable about relationships), (2) reciprocity (e.g., feeling obliged to help a computer that provided assistance), and (3) politeness norms (e.g., giving more favorable evaluations when the computer itself asks for feedback) \citep{nass1997machines}. 

A related concept is \textit{social presence}, which refers to ``the degree of salience of the other person in the interaction'' (p. 65) \citep{short1976social}. In the CA context, social presence captures the extent to which users perceive the agent as a real social entity rather than merely a tool \citep{oh2018systematic}. While CASA explains the automatic process of applying social rules, social presence addresses the outcome---the subjective sense of interacting with another being. Social presence is associated with increased trust and engagement \citep{gunawardena1995social}. Recent research raises questions about whether CASA effects persist as users become familiar with technology. Gambino et al. (\citeyear{gambino2020building}) proposed extending CASA, arguing that users may develop distinct human-media scripts over time. Furthermore, a replication study found that participants no longer responded socially to desktop computers after 30 years of exposure \citep{koger2023casa}. This suggests that CASA effects may be strongest for emergent technologies---a category into which LLM-based CAs currently fall. For CA design, CASA implies that even minimal anthropomorphic cues can trigger social responses. The question is not whether to anthropomorphize, but which cues to implement and how, thus motivating RQ1 and RQ2. These aspects have thus far eluded mainstream research in IS; as eloquently put by Peter et al., while much research attention is given to improving LLMs' reasoning capabilities, ``the progress in communicative abilities remains underappreciated'' (p. 1).
...
\subsection{Implementing Anthropomorphism in CAs and How it Affects User Outcomes}
Implementing anthropomorphism in CAs involves technical decisions that determine how users experience human-like characteristics. These decisions span three areas: how anthropomorphic attributes are represented, how consistency is maintained, and how the system adapts to users.
...
Despite these technical strides, there is a gap between \textit{technical implementation} (how to build it) and \textit{IS design} (how users perceive it). Current research rarely categorizes implementation methods (e.g., explicit prompting vs. implicit fine-tuning) in a way that helps IS practitioners make informed design choices to optimize anthropomorphic features for the CA's interaction with human users.
%\subsection{User Outcomes of CA Anthropomorphization}
In a similar vein, research on anthropomorphic CAs examines various user outcomes, yet these findings remain scattered across studies without systematic organization. Individual studies report effects on constructs such as trust \citep{nordheim2019trust}, satisfaction \citep{haugeland_understanding_2022}, engagement \citep{mehra_chatbot_2021}, and learning \citep{zheldibayeva_genai_2025}. %, but there is no clear understanding of what types of outcomes anthropomorphization actually affects.

Research on anthropomorphic IS shows that the effects of anthropomorphism extend beyond simple preference to affect actual system usage and perceived trust \citep{pfeuffer2019anthropomorphic,adam2021ai} through \textit{social presence}; that is, the perception of the agent as a ``real'' social entity rather than merely a tool \citep{meyer2023leverage,wang2021seeing}. For example, customer service research demonstrates that anthropomorphism increases engagement and purchase intentions through higher customer involvement \citep{depaula2025anthropomorphism,luo2019frontiers}.

However, the relationship is not straightforward. Excessive human-likeness can trigger \textit{uncanny valley} effects in which the CA too closely resembles a human, causing feelings of eerieness \citep{mori_bukimi_1970,soni2024chatbot}. Furthermore, mismatches between human-like cues and actual capabilities of a CA lead to expectancy violations that hamper satisfaction more severely than purely functional failures (i.e. the functionality of a system fails) \citep{ischen2024communication,qu2024chatbot}. Other commonly mentioned challenges in CA design include lacking conversational abilities and unappealing appearances \citep{diederich_design_2022}, as well as the risk of manipulation \citep{chaves2021should}.

Overall, the empirical findings on the effects of anthropomorphism are scattered and mixed. This lack of organization creates two issues. First, findings often appear contradictory because studies measure different outcomes using different instruments. For example, anthropomorphic design may increase trust \citep{go2019humanizing,konya2023anthropomorphism} but decrease compliance when users perceive AI personalization as intrusive \citep{chen_you_2021}. Without a systematic categorization, it is unclear whether these represent genuine contradictions or simply different outcome types responding differently to anthropomorphization. Second, practitioners lack guidance on how to design the anthropomorphic attributes of CAs for specific outcomes in specific contexts. For example, a CA designed for customer service may require different anthropomorphic strategies than one designed for education or healthcare, but current research does not map which design choices affect which outcomes.

Existing reviews have examined anthropomorphism's effects on specific outcomes such as trust \citep{bach2024systematic} or have focused on technical implementation \citep{chen2024trends}, but the field is missing the ``bigger picture'' of empirical consequences of the anthropomorphic design of CAs. The outcome types that anthropomorphization influences---and the conditions under which effects are positive, negative, or null---remain to be systematically identified. RQ3 addresses this gap by combining empirical findings across studies.
...
As illustrated in Table \ref{tab:lit_summary}, prior literature has extensively mapped the \textit{outcomes} of anthropomorphism, ranging from increased trust \citep{go2019humanizing} to potential backlash in failure scenarios \citep{crolic2022blame}. However, a critical gap remains in the CA design phase. 

First, the scattered attributes (visual, verbal, behavioral) identified across disparate studies have not been consolidated, leaving researchers without a clear picture of which anthropomorphic features are commonly used and which remain underexplored (addressed by \textbf{RQ1}). Second, while studies like \citet{zhang2018personalizing} address technical implementation, they are rarely connected to IS design for organizational use (addressed by \textbf{RQ2}). Finally, conflicting findings, where anthropomorphism improves outcomes in some studies \citep{go2019humanizing} but produces negative effects in others \citep{crolic2022blame}, suggest that moderating factors determine effectiveness, yet these conditions have not been systematically identified (addressed by \textbf{RQ3}).
% \begin{table}[htbp]
%     \centering
...
This systematic review of 79 empirical articles examined how anthropomorphism is operationalized in CA research. Three main findings emerged. First, the research community prioritizes behavioral anthropomorphism, communication style (50.0\% of articles applying) and personality traits (48.3\%), over visual appearance (15.5\%) or demographic identity (37.9\%). Second, anthropomorphic attributes cluster into two design approaches: a behavioral template (62.8\%) and an identity template (37.2\%). Third, user outcomes fall into four categories---user experience, performance, trust, and behavioral factors---with effects varying based on implementation choices. These findings offer researchers a theoretical foundation for attribute selection and provide practitioners with evidence-based design templates and guidelines for CA development.
