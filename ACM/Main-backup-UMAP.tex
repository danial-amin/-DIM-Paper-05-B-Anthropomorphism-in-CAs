...
\acmConference[UMAP '26]{User Modeling, Adaptation and Personalization}{June 08--11,
...
\title[The Effects of Anthropomorphism in Conversational Agents]{How Does Anthropomorphism Affect User Outcomes?: A Meta-Analysis of Conversational Agents}
...
Anthropomorphic design in conversational agents (CA) is of prime importance because it greatly shapes the nature of user interaction. However, the field lacks clear guidance on which types of anthropomorphic attributes are most important. We conducted a meta-analysis of 583 effects from 79 studies (2018-2025) examining how anthropomorphic attributes (behavioral vs. demographic) affect user outcomes, such as user experience (UX), trust, and credibility. The results show that behavioral attributes, such as communication style, personality traits, and responsiveness, create stronger effects on user outcomes than demographic attributes, such as demographic identity and visual appearance. We observe large effects of anthropomorphism on social perception and UX, and moderate effects on trust and learning. Our findings suggest that developers of CAs should prioritize how CAs communicate and behave over how they look or what they represent.
...
The design of these CA systems, particularly whether to assign human-like characteristics, which is referred to as \textit{anthropomorphism}, represents a critical decision affecting user acceptance, system effectiveness \citep{pfeuffer2019anthropomorphic,ashfaq2020deploying}, and engagement \citep{gong_how_2008}. As \cite{chandra_be_2022} argues, ``the business impact of conversational AI is contingent on customers using and adequately engaging with these tools'' (p. 969). Developments in LLMs exemplify the importance of anthropomorphism for engagement; for instance, OpenAI describes ChatGPT 5.1 as ``warmer'' and ``having more personality'' \citep{hayden_field_openai_2025}, demonstrating that leading AI organizations focus on anthropomorphism. Given that ChatGPT has more than 900M weekly active users as of December 2025\footnote{\url{https://backlinko.com/chatgpt-stats}}, the design choices in CAs command an immense impact.
...
\begin{figure*}[htbp]
    ...
    \begin{subfigure}[b]{0.33\textwidth}
        \centering
        \includegraphics[width=0.75\textwidth]{Figures/14.png}
...
Anthropomorphism in CAs (Figure \ref{fig:CAs}) usually involves assigning human characteristics such as names, personality traits, appearances, and conversational styles \citep{epley2007seeing,klein_effects_2025}. Research has expanded rapidly since the introduction of LLMs \citep{chen2024trends}, examining CAs' effects on conversation fluency, user experience (UX), and interaction outcomes. Some research demonstrates that anthropomorphic attributes increase trust through social presence \citep{konya2023anthropomorphism,blut2021understanding}, while other studies identify negative effects, such as human-like characteristics creating unrealistic expectations \citep{crolic2022blame,ciechanowski2019shades}.

Despite growing interest, a \textit{systematic understanding of which anthropomorphic attributes matter for user outcomes, such as UX, satisfaction, trust, and performance, remains lacking}. We define user outcomes as any measurable user response to CA interactions, including perceptions, attitudes, behaviors, and performance. Studies examine individual characteristics, such as names \citep{go2019humanizing}, gender, communication style \citep{seeger2021texting}, but findings remain scattered, and organizations deploying conversational interfaces lack evidence-based guidance for design decisions. For example, trust formation is a primary concern, as users must decide whether to accept CAs \citep{meyer2023leverage,følstad2021chatbots,nordheim2019trust}. Without the synthesis of anthropomorphic CA attributes, implementation methods, and user outcomes, CA developers make design choices based on intuition rather than validated research.

Prior meta-analyses have examined human-like cues in conversational agents. Chavez and Gerosa \cite{chaves2021should} reviewed 56 articles to analyze how social characteristics influence human–chatbot interactions, finding that certain CA characteristics are domain-dependent (e.g., moral agency, communicability), whereas others apply broadly across contexts (e.g., manners, damage control). Klein \cite{klein_effects_2025} synthesized 142 studies on human-like social cues in text-based conversational agents, reporting a small-to-medium effect ($g$ = 0.36) on social responses. They identified variability across anthropomorphic attributes and outcome measures, though the study focused exclusively on text-based interactions. 

Building on this foundation, we extend this line of inquiry by examining anthropomorphic attributes across all CA modalities (text, voice, embodied), distinguishing between behavioral (communication style, personality, responsiveness) and demographic attributes (demographic identity, role, visual appearance). %, and employing hybrid synthesis methods to maximize data utilization.
This meta-analysis addresses the following research question: 
\textit{How do anthropomorphic attributes of conversational agents affect user outcomes?} 
We synthesize empirical findings from 79 articles (2018-2025), organizing results by outcome categories and identifying which specific attributes produce consistent effects. Our analysis focuses on a meta-analysis of 583 effects ( N = 21,104). % with qualitative synthesis methods (365 additional effects), achieving 93\% data utilization through vote counting \cite{votecounting}, Fisher's combined probability test, and Stouffer's Z-score methods.

Understanding how anthropomorphic attributes affect user outcomes is essential for evidence-based CA design. Research reports effects on trust and acceptance \citep{kraus_role_2021,chandra_be_2022}, UX \citep{lieb_student_2024,liu_older_2023}, behavioral patterns \citep{mehra_chatbot_2021,chen_you_2021}, and task performance \citep{zheldibayeva_genai_2025,baihaqi_rapport-driven_2024}. However, findings are scattered. Our synthesis organizes these empirical findings into outcome categories and identifies conditions under which anthropomorphic design produces effects.

This meta-analysis makes at least three contributions to the user modeling (UM), adaptation, and personalization community. First, we cluster anthropomorphic attributes into behavioral and demographic groups using hierarchical clustering analysis. Second, we demonstrate that behavioral attributes produce substantially stronger effects than demographic attributes, with overall moderate effects approximately twice the magnitude of prior text-only meta-analyses. Third, we synthesize findings across outcome categories, revealing the strongest effects for social perception and UX, with smaller but significant effects for behavioral outcomes. Fourth, we provide design guidelines for the future CA developers to design purpose built anthropomorphic CAs.
...
Across these different types, a shared trait is the reliance on natural language for interaction \citep{diederich_design_2022}. 
Another key trait of CAs is that, unlike navigation-based UIs, CAs adapt to user needs \citep{allouch2021conversational,jain2018evaluating}. For example, a student can ask a CA to provide steps to solve a mathematics problem, and the CA would do so. A CA can take multiple forms, including static or interactive virtual avatars, physical embodiments, or a disembodied form that relies solely on natural language \citep{diederich_conversational_2019,diederich_design_2022}. These different implementations shape how users perceive the CA \citep{peter_benefits_2025}, while making systems and information more accessible. Peter et al. \cite{peter_benefits_2025} even argue that one of the core objectives of computer science is to ``to make computers more accessible and more natural to interact with, \textit{by making computing more human-like}'' (p. 1, emphasis by us). Chavez and Gerosa \cite{chaves2021should} further emphasize the role of interaction design by postulating that ``making a conversational agent acceptable to users is primarily a social, not only technical, problem'' (p. 729).
...
\subsection{Theory of Anthropomorphism}
...
A related concept is \textit{social presence}, which refers to ``the degree of salience of the other person in the interaction'' (p. 65) \citep{short1976social}. In the CA context, social presence captures the extent to which users perceive the agent as a real social entity rather than merely a tool \citep{oh2018systematic}. While CASA explains the automatic process of applying social rules, social presence addresses the sense of interacting with another being. Social presence is associated with increased trust and engagement \citep{gunawardena1995social}. Prior research raises questions about whether CASA effects persist as users become familiar with technology. Gambino et al. \cite{gambino2020building} proposed extending CASA, arguing that users may develop distinct human-media scripts over time. Furthermore, a replication study found that participants no longer responded socially to desktop computers after 30 years of exposure \citep{koger2023casa}. This suggests that CASA effects may be strongest for emergent technologies, such as LLM-based CAs. CASA implies that even minimal anthropomorphic cues can trigger social responses. Therefore, the question is not whether to anthropomorphize or not, but \textit{how}.  %, thus motivating RQ1 and RQ2. These aspects have thus far eluded mainstream research in IS; as eloquently put by Peter et al., while much research attention is given to improving LLMs' reasoning capabilities, ``the progress in communicative abilities remains underappreciated'' (p. 1).
...
\subsection{Effects of Anthropomorphism on Users}
...
Research shows that anthropomorphism affects system usage and perceived trust \citep{pfeuffer2019anthropomorphic,adam2021ai} through perceiving the agent as a ``real'' social entity rather than a ``tool'' \citep{meyer2023leverage,wang2021seeing}. For example, anthropomorphism in customer service CAs increases engagement and purchase intentions \citep{depaula2025anthropomorphism,luo2019frontiers}. Researchers have also found that CAs' anthropomorphism increases perceptions of competence and warmth, which in turn support empathy and increase users' willingness to adopt CAs \citep{kim_what_2024}.
...
Overall, the empirical findings on the effects of anthropomorphism are scattered and mixed, such that anthropomorphism improves outcomes in some studies \citep{go2019humanizing} but produces negative effects in others \citep{crolic2022blame}. For example, anthropomorphic design may increase trust \citep{go2019humanizing,konya2023anthropomorphism}, yet decrease compliance when users find personalization intrusive \citep{chen_you_2021}. Without systematic analysis, we cannot determine whether such findings genuinely contradict each other or whether user outcomes simply respond differently to anthropomorphization.
 %Second, practitioners lack guidance on how to design the anthropomorphic attributes of CAs for specific outcomes in specific contexts. For example, a CA designed for customer service may require different anthropomorphic strategies than one designed for education or healthcare, but current research does not map which design choices affect which outcomes.

Previous reviews have examined anthropomorphism's effects on specific outcomes such as trust \citep{bach2024systematic} or have focused on technical implementation \cite{chen2024trends}, but the field is missing the ``bigger picture'' of empirical consequences of the anthropomorphic design of CAs. While Klein's \cite{klein_effects_2025} meta-analysis established that human-like social cues in text-based agents produce positive effects, this work did not distinguish between behavioral and demographic attributes, examine voice or multimodal agents, or analyze which specific attributes work best for which outcomes. This expansion of CAs beyond text-based agents and the targeted effects of specific behavioral and demographic attributes on user outcomes are the subject of this study.  %RQ3 addresses this gap by combining empirical findings across studies.

% Table \ref{tab:lit_summary} illustrates the effects of anthropomorphism in CAs. %This synthesis reveals that while the impact of human-like design on user psychology (e.g., trust, satisfaction) is well-researched, the specific configurations of attributes and their technical implementations remain varied and often disconnected from the interaction context.
% As illustrated in Table \ref{tab:lit_summary}, prior literature has extensively mapped the \textit{outcomes} of anthropomorphism, ranging from increased trust \citep{go2019humanizing} to potential backlash in failure scenarios \citep{crolic2022blame}. However, the scattered attributes (visual, verbal, behavioral) identified across disparate studies have not been consolidated, leaving researchers without a clear picture of which anthropomorphic features are commonly used and which remain underexplored. Also, conflicting findings, where anthropomorphism improves outcomes in some studies \citep{go2019humanizing} but produces negative effects in others \citep{crolic2022blame}, suggest that these conditions have not been systematically identified, which we aim to carry out in the current study.
% \begin{table}[htbp]
%     \centering
%     \caption{Summary of Key Studies on Anthropomorphism in CAs}
%     \label{tab:lit_summary}
%     \renewcommand{\arraystretch}{1.3}
%     \footnotesize
%     \begin{tabular}{p{0.15\textwidth} p{0.15\textwidth} p{0.25\textwidth} p{0.35\textwidth}}
%         \toprule
%         \textbf{Study} & \textbf{Context} & \textbf{Attributes Studied} & \textbf{Key Findings} \\
%         \midrule
%         \citet{go2019humanizing} & General Chat & Identity cues (name, gender), Visual cues & Visual anthropomorphism increases perceptions of social presence, leading to higher system acceptance. \\
%         \midrule
%         \citet{crolic2022blame} & Customer Service & Human-like voice, Apology style & High anthropomorphism in failure contexts increases user anger due to expectancy violations. \\
%         \midrule
%         \citet{liu2024exploring} & Service Failure & Empathy, Warmth, Competence & Anthropomorphism mitigates blame attribution during service failures by triggering social norms of forgiveness. \\
%         \midrule
%         \citet{araujo2018living} & Corporate Comms & Language style, Social cues & Cues in dialogue are more effective than visual avatars in establishing social presence. \\
%         \midrule
%         \citet{seeger2021texting} & Conversational Commerce & Emojis, Typographic errors, Response time & Subtle conversational cues (e.g., waiting times) significantly enhance perceived human-likeness without explicit avatars. \\
%         \midrule
%         \citet{zhang2018personalizing} & Technical / NLP & Explicit Persona Profiles (PersonaChat) & Conditioning models on explicit persona profiles improves consistency and user engagement compared to generic models. \\
%         \bottomrule
%     \end{tabular}
% \end{table}
...
% \begin{table*}[htbp]
%     \centering
%     \caption{\textbf{Example studies on anthropomorphism in conversational agents.} Researchers have studied many aspects of anthropomorphism in CAs, both as an independent and a dependent variable.}
%     \label{tab:lit_summary}
%     \renewcommand{\arraystretch}{1.3}
%     \footnotesize
%     \begin{tabular}{p{0.15\textwidth} p{0.15\textwidth} p{0.6\textwidth}}
%         \toprule
%         \textbf{Study} & \textbf{Context} & \textbf{Key Findings} \\
%         \midrule
%         \citet{go2019humanizing} & General Chat & Visual anthropomorphism increases perceptions of social presence, leading to higher system acceptance. \\
%         \midrule
%         \citet{crolic2022blame} & Customer Service & High anthropomorphism in failure contexts increases user anger due to expectancy violations. \\
%         \midrule
%         \citet{liu2024exploring} & Service Failure & Anthropomorphism mitigates blame attribution during service failures by triggering social norms of forgiveness. \\
%         \midrule
%         \citet{araujo2018living} & Corporate Comms & Cues in dialogue are more effective than visual avatars in establishing social presence. \\
%         \midrule
%         \citet{seeger2021texting} & Conversational Commerce & Subtle conversational cues (e.g., waiting times) significantly enhance perceived human-likeness without explicit avatars. \\
%         \midrule
%         \citet{zhang2018personalizing} & Technical / NLP & Conditioning models on explicit persona profiles improves consistency and user engagement compared to generic models. \\
%         \bottomrule
%     \end{tabular}
% \end{table*}

% \subsection{Conversational Agents as Information Systems}

% CAs function as interactive interfaces that collect, process, and disseminate information through natural language \citep{nguyen2024conversational,mariani2023artificial,pfeuffer2019anthropomorphic}. Not relying on navigable user interfaces, CAs support dynamic, context-aware information exchange, adapting to user needs \citep{allouch2021conversational,jain2018evaluating}. For example, a student can ask a CA to provide steps to solve a mathematics problem and the CA would do so. To accommodate such interactions, IS design has evolved from \textit{form-based} format to \textit{conversational interaction} \citep{finch2023dont,dam2024complete}. In designing these interactions, it is vital to investigate why people engage with CAs and how design choices affect the adoption of CA systems \citep{brandtzaeg2017people,ashfaq2020deploying}.

% Modern CAs integrate with enterprise systems, including customer relationship management (CRM) systems, knowledge management platforms, and transactional databases \citep{mariani2023artificial}. CAs can be used to interact with the organizational information assets \citep{nguyen2024conversational}. Modern CAs not only fetch information but can also generate knowledge, thus making them as active participants in information flow \citep{allouch2021conversational}.

% Contemporary CAs are increasingly being used in specialized contexts, from healthcare support and mental health interventions to educational tutoring and customer service automation \citep{mariani2023artificial,nguyen2024conversational}. Multimodal capabilities support interactions through text, voice, and visual interfaces in messaging applications, smart speakers, and mobile devices \citep{allouch2021conversational}. Cloud-based architectures support scalable deployment, featuring dialogue management, sentiment analysis, and context preservation \citep{schobel2024charting}. So, the potential value of CAs in the enterprise sector appears evident.

% The shift toward transformer architectures and LLMs has dramatically improved CA capabilities \citep{finch2023dont}. Whereas pre-LLM conversational systems relied on rule-based pattern matching, LLM CAs generate novel responses and maintain coherent multi-turn conversations without predefined scripts \citep{fu2022learning}. However, this evolution introduces challenges for providing consistent quality, preventing hallucinations, and maintaining appropriate boundaries \citep{dam2024complete}.

% \subsection{The Effects of Anthropomorphism on CAs}

% Anthropomorphism is the attribution of human characteristics, emotions, or behaviors to non-human entities such as CAs. Anthropomorphic design affects critical user outcomes, including trust formation, engagement levels, and behavioral compliance \citep{meyer2023leverage,ischen2024communication}. This anthropomorphism operates through social presence, such as users perceive the agent as a social entity rather than merely a tool \citep{meyer2023leverage,wang2021seeing}. Features including anthropomorphism through avatars and socially-oriented communication styles both contribute independently to this effect. Research on anthropomorphic information systems shows that these effects extend beyond simple preference to affect actual system usage and compliance behaviors \citep{pfeuffer2019anthropomorphic,adam2021ai}.

% Customer service research demonstrates that anthropomorphism positively influences engagement and purchase intentions by increasing customer involvement \citep{depaula2025anthropomorphism,luo2019frontiers}. However, the relationship is not straightforward. Excessive human-likeness can trigger uncanny valley effects, degrading brand attitudes \citep{soni2024chatbot}. The perception of intelligence and competence varies with anthropomorphic presentation, as users attribute greater capability to human-like systems, even when actual performance remains constant \citep{skjuve2021my,mozafari2022anthropomorphic}. Anthropomorphic features build social presence but risk creating unrealistic expectations.

% %Failure scenarios add additional dimensions. 
% When CAs fail to meet expectations, anthropomorphic characteristics can restore trust by allowing users to apply human relationship frameworks, reducing blame attributions \citep{liu2024exploring}. Yet this protective effect depends on individual differences—AI anxiety and technology self-efficacy moderate responses to anthropomorphic features \citep{liu2024exploring,wang2025influence}. Mismatches between human-like cues and actual capabilities lead to expectancy violations that negatively affect satisfaction more severely than purely functional failures \citep{ischen2024communication,qu2024chatbot}.

% Technical advances address key challenges in maintaining consistent persona representations throughout extended interaction. Research has progressed beyond simple profile conditioning to incorporate dynamic persona extraction and memory management, allowing systems to track and update persona information over time \citep{xu2022long,maharana2024evaluating}. Dialogue act-based methods extract partner personas from conversational context, recognizing that effective anthropomorphism requires understanding both agent characteristics and adapting to interlocutor attributes \citep{kim2024dialogue}.

% CA consistency has been addressed through reinforcement learning approaches optimizing coherence between responses and predefined attributes and natural language inference models verifying alignment between utterances and profiles \citep{shea2023building,yan2023personalized}. Long-term memory mechanisms allow agents to maintain coherence in multiple sessions through selectively retrieving relevant historical information \citep{xu2022long,maharana2024evaluating}.

% LLM integration into CAs creates new possibilities while introducing novel challenges. LLMs support the creation of synthetic persona-based datasets, augmenting limited human-annotated resources through iterative refinement, ensuring faithfulness to defined attributes \citep{jandaghi2023faithful}. These models implement persona-grounded dialogue through prompt engineering and retrieval-augmented generation without requiring task-specific fine-tuning \citep{ahn2023mpchat}. However, evaluations show LLMs struggle to maintain consistent persona identities over very long conversations, with performance degrading as dialogue length and persona complexity increase \citep{maharana2024evaluating,lee2025examining}. This limitation motivates hybrid architectures combining LLM capabilities with specialized memory modules and verification mechanisms \citep{zhang2024hello,lee2025examining}. Design taxonomies for service CAs have begun cataloging implementation patterns, identifying feature combinations that prove effective across contexts \citep{diederich2022design,kim2019systematic}.

% % \subsection{Ethical Considerations in Anthropomorphic Design}

% % Anthropomorphic design raises ethical concerns that extend beyond technical effectiveness to questions of deception, bias, and appropriate boundaries in human-AI relationships \citep{völkel2020eliciting,sundar2020rise}. The assignment of human-like characteristics risks misleading users about system capabilities, particularly when conversational fluency exceeds actual understanding or when empathetic responses suggest emotional comprehension that systems lack \citep{araujo2018living,luo2019frontiers}. Demographic attribute assignment, particularly gender, can reinforce occupational stereotypes and exclude non-binary identities \citep{go2019humanizing}. 

% % The disclosure question proves particularly acute: should CAs explicitly identify themselves as artificial, or does transparent disclosure undermine the social presence benefits that anthropomorphism provides? \citep{luria2019judging,nordheim2019trust}. Healthcare and financial contexts demand transparency given consequential decision-making, while entertainment contexts may allow ambiguity \citep{araujo2018living}. Research examining user relationships with CAs shows that prolonged interaction can create inappropriate attachments or overreliance \citep{skjuve2021my}, suggesting the need for design boundaries. Research on appropriate boundaries remains limited, with most articles examining effectiveness rather than ethical implications of creating human-like AI agents that users may form attachments to or trust without due cause.


% \section{Methodology}

% This systematic review followed established guidelines from other IS research applying the systematic review approach \citep{collins2021artificial,hummel2014state,epizitone2023systematic}. Our four-phase methodology comprised: (1) comprehensive database search, (2) multi-stage screening with quality assessment, (3) systematic data extraction using structured coding, and (4) thematic analysis. The protocol was designed to ensure reproducibility and can be accessed in the online appendix (OSF \footnote{\href{https://osf.io/nzbu8/overview?view_only=4a155351aa3a44849a5af3ae0c15b458}{https://osf.io/nzbu8/overview?view\_only=4a155351aa3a44849a5af3ae0c15b458}}).

% \subsection{Search Query}

% We searched four major databases: (i) Scopus, (ii) Web of Science, (iii) IEEE Xplore Digital Library, and (iv) ACM Digital Library, which are databases that cover relevant IS conferences and journals. The search was conducted in August 2025, covering January 2018 through August 2025. This time frame captures contemporary research following the influential PersonaChat dataset \citep{zhang2018personalizing} and the subsequent proliferation of persona-based dialog systems.

% The search query was created to capture articles that address both CAs and persona-related concepts. The query was created by combining commonly used words for CAs and personas in the CA context. The words were selected from the topical search in Google Scholar \footnote{\url{https://scholar.google.com/schhp?hl=en}} and combining the most common keywords. The query was further refined iteratively over a course of four iterations, based on thorough discussions between the principal investigator (PI) and co-principal investigator (Co-PI). % of the article, who collectively have an experience of over two decades, conducting research. 
% The Boolean search string used across all databases was:
...
% This query structure combines `OR' operators to capture the comprehensiveness of CAs and persona traits, with the `AND' operator ensures that articles address both core concepts. Wildcard operators (e.g., persona*) were employed to capture variations of search terms. The search was limited to article titles, abstracts, and keywords to maintain relevance. The initial combined search yielded 735 unique publications after removing duplicates.

% \subsection{Article Selection}

% The selection of articles included several filtering steps to include pertinent, high-quality primary articles in the final corpus. Figure~\ref{fig:prisma} presents the PRISMA (Preferred Reporting Items for Systematic reviews and Meta-Analyses) flow diagram illustrating the selection process.

% \begin{figure}
%     \centering
%     \includegraphics[width=\linewidth]{Figures/PRISMA CA Anthropomorphism.jpg}
%     \caption{\textbf{Screening procedure.} The PRISMA diagram shows the different screening steps to select the articles for the corpus.}
%     \label{fig:prisma}
% \end{figure}

% \subsubsection{Quality Assessment}

% We employed the Finnish national JUFO (Publication Forum\footnote{``Julkaisufoorumi'' in Finnish.}) ranking system to assess publication venue quality \citep{jufo2021classification} due to the fact that this study was carried out in a Finnish research institute. JUFO is a Finnish publication forum rating system that classifies scholarly publication venues (e.g., journals, book publishers, and conferences) into four quality levels (from 0 to 3, higher indicating better) based on a juried, professor-led process in order to help evaluate research quality. Only articles published in venues rated JUFO level 1 or higher were included, ensuring a baseline standard of peer review and scholarly rigor. For venues not indexed in the JUFO database, the PI assessed quality based on venue indexing (e.g., Scopus, Web of Science coverage), impact factor, and established reputation within the research community. This assessment was conducted through discussion among the research team to reach consensus.

% \subsubsection{Screening Process}

% The screening process consisted of two stages. First, title and abstract screening removed clearly irrelevant publications. This stage eliminated conference reviews, book chapters, and articles not meeting basic relevance criteria, reducing the pool from 670 (after deduplication) to 620 publications.

% Second, full-text screening was conducted using detailed inclusion and exclusion criteria. Researchers independently reviewed each publication, with disagreements resolved through discussion. The inclusion criteria were:

% \begin{itemize}
%     \item \textbf{I1: Language}: Published in English
%     \item \textbf{I2: Publication type}: Peer-reviewed primary research (excluding review articles, opinion pieces, and secondary articles)
%     \item \textbf{I3: Focus on CAs}: The article must investigate CAs, dialogue systems, or CAs as the primary focus
%     \item \textbf{I4: Anthropomorphic implementation}: The conversational agent must have an explicitly/implicitly defined and/or implemented persona with specific attributes (e.g., personality traits, demographic characteristics, communication style)
%     \item \textbf{I5: Empirical evaluation}: The article must include an evaluation of the anthropomorphic CA with well defined dependent and independent variables.
% \end{itemize}

% Articles were excluded if they: (1) focused solely on technical architecture without persona characteristics; (2) discussed personas only conceptually without implementation or evaluation; (3) examined user personas rather than agent personas; or (4) lacked sufficient methodological detail for quality assessment.
% After applying these screening criteria, 98 articles (12.1\%) remained. Furthermore, during data encoding, an additional 19 articles were removed due to a lack of information, alternative details, or concerns about the methodology. This resulted in a total of 79 articles (10.1\%). Table~\ref{tab:screening_summary} presents the number of articles retained at each screening stage.

% \begin{table*}[htbp]
% \centering
% \caption{\textbf{Article screening summary.} Various stages were included to ensure quality and relevance of the included works.}
% \label{tab:screening_summary}
% \begin{tabular}{lcc}
% \hline
% \textbf{Screening Stage} & \textbf{Articles Removed} & \textbf{Articles Remaining (\% of total)} \\
% \hline
% Combined collection & -- & 735 (100\%) \\
% Deduplication (automatic) & 65 & 670 (91.1\%) \\
% Deduplication (manual) & 31 & 639 (86.9\%) \\
% Conference reviews \& book chapters & 19 & 620 (84.3\%) \\
% \hline
% \multicolumn{3}{l}{\textit{Full-text screening with inclusion criteria:}} \\
% Quality assessment (JUFO) & 59 & 561 (76.3\%) \\
% Language (English) & 0 & 561 (76.3\%) \\
% Peer-reviewed primary article & 16 & 545 (74.1\%) \\
% Focus on CAs & 111 & 450 (61.2\%) \\
% Persona defined/implemented & 227 & 223 (30.3\%) \\
% Evaluation included & 125 & 98 (13.3\%) \\
% \hline
% \multicolumn{3}{l}{\textit{Encoding Stage Exclusion}} \\
% Lack of Information & 12 & 86 (11.7\%) \\
% Alternative Focus & 3 & 83 (11.2\%) \\
% Concerns about methodology & 4 & \textbf{79 (10.7\%)} \\
% \hline
% \end{tabular}
% \end{table*}
...
% Data extraction was performed using a structured coding book developed iteratively by the PI and co-PI in collaboration. The coding scheme captured multiple dimensions of persona-based CAs, including persona attributes, implementation characteristics, article design features, and application contexts, with the procedure detailed in the following subsections.

% \subsubsection{Coding Book Development}

% The coding book was developed through an iterative process, inspired by the dimensions of a previous systematic review of CAs in IS research \citep{diederich_conversational_2019}. The PI and co-PI first created a coding book based on the RQs determined in the initial phase of the research. The PI and co-PI then reviewed a sample of 5 articles to identify the gaps and challenges in the process. The coding book was then revised to reduce the friction. This initial review informed the development of a preliminary coding book, which contains variable definitions, coding instructions, and examples. The coding book was refined through pilot coding of an additional 5 articles, with discrepancies discussed and resolved to clarify ambiguous coding decisions. The final coding scheme included 30 variables organized into the following categories:

% \begin{itemize}
%     \item \textbf{Article characteristics}: Publication year, document type, field of article, research methodology \citep[adapted from IS research method dimension in][]{diederich_conversational_2019}, data type (quantitative, qualitative, or mixed methods)
%     \item \textbf{Anthropomorphic attributes}: Human name assignment, gender assignment, persona attributes list, backstory provision, digital twin representation (representing a specific real human)
%     \item \textbf{Implementation features}: Self-reference method (first-person, third-person), visual representation \citep[extending CA embodiment dimension in][]{diederich_conversational_2019}, communication modality \citep[extending CA communication mode in][]{diederich_conversational_2019}, persona disclosure to users, fixed versus adaptive persona, interaction initiation, politeness encoding
%     \item \textbf{Context variables}: Application domain \citep[adapted from CA context in][]{diederich_conversational_2019}, organizational development context, intended user audience, interaction purpose (advisory, educational, professional use, companionship, entertainment, informational)
%     \item \textbf{Anthropomorphism approach}: How anthropomorphization was implemented, description of human-like characteristics
% \end{itemize}

% % \begin{itemize}
% %     \item \textbf{Article characteristics}: Publication year, document type, field of article, research methodology (empirical or conceptual), data type (quantitative, qualitative, or mixed methods)
% %     \item \textbf{Anthropomorphic attributes}: Human name assignment, gender assignment, persona attributes list, backstory provision, digital twin representation (representing a specific real human)
% %     \item \textbf{Implementation features}: Self-reference method (first-person, third-person), visual representation (text-only, avatar/embodied), communication modality (text, voice, multimodal), persona disclosure to users, fixed versus adaptive persona, interaction initiation, politeness encoding
% %     \item \textbf{Context variables}: Application domain, organizational development context, intended user audience, interaction purpose (advisory, educational, professional use, companionship, entertainment, informational)
% %     \item \textbf{Anthropomorphism approach}: How anthropomorphization was implemented, description of human-like characteristics
% % \end{itemize}

% The complete coding book and operational definitions for each variable are available in the supplementary materials \footnote{\href{https://osf.io/nzbu8/overview?view_only=4a155351aa3a44849a5af3ae0c15b458}{https://osf.io/nzbu8/overview?view\_only=4a155351aa3a44849a5af3ae0c15b458}}.

% \subsubsection{Coding Process and Reliability}

% Five trained coders (with a combined experience of over 6 decades in research) independently extracted data from the included articles. Each coder received clear instructions on the coding book. To ensure reliability, 10\% of articles were double-coded by two independent coders. Inter-rater reliability was assessed using Cohen's kappa for 12 categorical coding variables across five independent coders ratings, achieved Cohen's kappa (mean $\kappa = 0.81$), indicating strong inter-rater agreement. Disagreements were resolved through discussion among coders and, when necessary, consultation with the PI. 

% For articles involving human participants, we documented whether the article conducted experiments with people using the CA system and systematically recorded the research findings and design considerations that emerged from the empirical evaluations.

% \subsection{RQ2: Operationalization and Implementation}

% \subsubsection{Persona Disclosure Strategies}

% Articles varied considerably in persona disclosure among the 58 anthropomorphisation articles. The majority, 34 articles (58.6\%), explicitly informed users about personality characteristics or role, while 15 articles (25.9\%) did not disclose persona attributes, allowing inference through interaction. The remaining 9 articles (15.5\%) did not specify disclosure approach.

% For example, in customer service framing, research explicitly cued agent role and anthropomorphic design, improving perceptions of both company and agent \citep{araujo2018living}. Other approaches induced human-like perceptions primarily through language design such as politeness and turn-taking without direct disclosure \citep{seeger2021texting}. \textbf{The predominance of explicit disclosure (58.6\%) suggests the field increasingly values transparency in CAs design.}

% \subsubsection{Fixed Versus Adaptive Personas}

% Most implementations used fixed personas consistent for all users and interactions (n=42, 72.4\%), while adaptive approaches modifying characteristics based on user interactions appeared in 14 articles (24.1\%). The remaining two articles (3.4\%) did not specify adaptability.

% Research has implemented persona-adaptive attention to update responses dynamically to user preferences \citep{huang2023personalized}. Other work grounded conversations in both persona and knowledge, adjusting outputs according to context for better user alignment \citep{jang2022focus}. 

% Longitudinal analysis showed adaptive personalization is increasing significantly over time (Spearman's $\rho = .83$, $p = .011$), suggesting growing technical capabilities and research interest. \textbf{Despite this upward trend, fixed personas still dominate (72.4\%), indicating adaptive personalization remains technically challenging.}

% \subsubsection{Visual Representation Patterns}

% Visual representation choices divided primarily between text-only interfaces (n=30, 51.7\%) and avatar/embodied implementations (n=21, 36.2\%), with the remainder employing other modalities (n=7, 12.1\%). Choice of visual representation showed domain-specific patterns: healthcare and education applications more frequently employed embodied agents compared to customer service implementations.

% Multimodal, persona-grounded systems often include avatar components \citep{ahn2023mpchat}. Identity and visual cues improve humanness perceptions \citep{go2019humanizing}. \textbf{The text-only dominance (51.7\%) reflects practical deployment constraints rather than theoretical preferences,} as embodied agents require greater technical resources.

% \subsubsection{Communication Modality Distribution}

% Text remained the dominant communication channel (n=35, 60.3\%), though multimodal implementations incorporating voice, gesture, or other modalities were substantial (n=19, 32.8\%). Pure voice-only interfaces were rare (n=3, 5.2\%).

% Avatar representation and voice capability demonstrated significant association ($\chi^2 = 3.96$, $p = .047$, $\phi = .26$), indicating a multimodal design pattern wherein embodied agents were more likely to incorporate voice than text-only systems. Among the 21 embodied agents, 12 (57.1\%) included voice capability, compared to only 10 of 37 (27.0\%) non-embodied systems. Research has explicitly combined visual and linguistic channels in persona-grounded dialogue \citep{ahn2023mpchat}. Other work showed that anthropomorphization co-occurring with socially oriented communication improves perceived social presence and outcomes \citep{meyer2023leverage}.

% \subsubsection{Interaction Initiation}

% Among the 58 anthropomorphization studies, 19 (32.8\%) described CAs initiating conversations or proactively engaging users, while 39 (67.2\%) specified user-initiated interactions. Unlike other implementation dimensions, all articles specified initiation patterns. \textbf{The predominance of user-initiated interactions (67.2\%) may reflect practical deployment considerations where proactive contact risks being perceived as intrusive.}

% \subsubsection{Digital Twin Implementations}

% Digital twin representations, which are CAs designed to mimic specific real individuals, were rare (n=6, 10.3\%). Most implementations (n=52, 89.7\%) used fictional personas not tied to particular individuals. Partner- and counterpart-focused persona extraction points toward individual-specific adaptation. Research has proposed dialogue-act-based partner persona extraction for consistent personalized responses \citep{kim2024dialogue}. \textbf{Digital twin implementations represent a frontier area with significant privacy and ethical implications.}

% Table~\ref{tab:impl} summarizes implementation strategies for key dimensions.

% \begin{table*}[t]
% \centering
% \caption{Implementation strategies of XXXX showing dominance of explicit disclosure (58.6\%), fixed personas (72.4\%), and text-only interfaces (51.7\%). The most common combination from our data is the following: XXXX. The most common category values bolded.}
% \label{tab:impl}
% \begin{tabularx}{\textwidth}{@{}l l c c X@{}}
% \toprule
% \textbf{Dimension} & \textbf{Category} & \textbf{n} & \textbf{\%} & \textbf{Notes} \\
% \midrule
% Persona disclosure & Explicit & 34 & 58.6 & Users informed about persona/role \\
%                   & Implicit & 15 & 25.9 & Persona inferred via interaction \\
%                   & Not specified & 9 & 15.5 & \\
% \addlinespace
% Adaptability & Fixed & 42 & 72.4 & Same persona for all users \\
%              & Adaptive & 14 & 24.1 & Persona adapts to user/context \\
%              & Not specified & 2 & 3.4 & \\
% \addlinespace
% Visual representation & Text-only & 30 & 51.7 & \\
%                       & Avatar/embodied & 21 & 36.2 & \\
%                       & Other & 7 & 12.1 & \\
% \addlinespace
% Communication modality & Text & 35 & 60.3 & \\
%                        & Multimodal & 19 & 32.8 & Includes voice and visual \\
%                        & Voice only & 3 & 5.2 & \\
% \addlinespace
% Interaction initiation & Chatbot-initiated & 19 & 32.8 & \\
%                        & User-initiated & 39 & 67.2 & \\
% \addlinespace
% Digital twin & Yes & 6 & 10.3 & Represents specific real person \\
%              & No & 52 & 89.7 & Fictional or general persona \\
% \bottomrule
% \end{tabularx}
% \end{table*}
...
% \subsection{RQ3: Application Domains and Contexts}

% \subsubsection{Domain Distribution}

% Among the 58 anthropomorphisation articles, persona-based CA research spanned multiple domains, with \textbf{information seeking emerging as the most studied} (n=12, 20.7\%), followed by customer service (n=8, 13.8\%), education (n=8, 13.8\%), and healthcare (n=8, 13.8\%). Social companionship (n=4, 6.9\%), entertainment (n=3, 5.2\%), and professional work (n=2, 3.4\%) comprised smaller proportions. A substantial ``Other'' category (n=13, 22.4\%) included diverse domains not fitting established categories.

% Customer service research examined identity and conversational cues to increase perceived humanness \citep{go2019humanizing}. In online health consultation, anthropomorphic presentation affects reuse intentions and interacts with disease severity \citep{qu2024chatbot}. Educational applications proposed persona-grounded dialogue to tailor interactions and engagement \citep{jang2022focus}.

% \subsubsection{HCI-NLP Methodological Divide}

% Analysis of the 48 articles clearly identified as either HCI (n=40) or NLP (n=8) showed a critical methodological divide. \textbf{HCI research demonstrated significantly higher human participant experiment rates} (39/40, 97.5\%) compared to NLP research (4/8, 50.0\%), yielding large effect size (Fisher's exact test: OR = 39.00, $p = .002$). This substantial difference reflects divergent epistemological orientations: HCI prioritizes user experience evaluation and behavioral outcomes, while NLP emphasizes technical implementation, model architecture, and computational metrics.

% HCI researchers employed controlled user articles testing anthropomorphic design effects on interaction outcomes \citep{seeger2021texting}. In contrast, NLP research evaluated persona consistency via offline reinforcement learning objectives and computational metrics without user experiments \citep{shea2023building}. The fields also differed in implementation choices: HCI articles more frequently incorporated avatar/embodied representations (15/40, 37.5\%) compared to NLP articles (1/8, 12.5\%), though this difference did not reach statistical significance (Fisher's exact test: $p = .14$).

% Figure~\ref{fig:hci-nlp} illustrates this methodological divide, which has important implications for knowledge integration between communities, as we discuss in Section \ref{discussion}.

% \begin{figure}[t]
%   \centering
%   \fbox{\parbox{0.92\linewidth}{\centering HCI vs. NLP comparison chart placeholder.}}
%   \caption{HCI vs. NLP comparison showing 97.5\% vs. 50.0\% human experiment rates and embodiment implementation differences (37.5\% vs. 12.5\%).}
%   \label{fig:hci-nlp}
% \end{figure}

\section{Methodology}

This meta-analysis examined the impact of anthropomorphic attributes in CAs on user outcomes. We synthesized empirical findings from a systematic literature review \cite{amin2025anthropomorphism} of 79 articles published between 2018 and 2025, employing an approach that combined quantitative meta-analysis with qualitative synthesis to maximize data utilization and address our research question comprehensively.

\subsection{Systematic Literature Review}

We conducted searches following PRISMA guidelines \cite{page_prisma_2021} in four academic databases: SCOPUS, Web of Science, IEEE Xplore, and ACM Digital Library. The search string combined CA terminology (``conversational agent*,'' ``chatbot*,'' ``virtual agent*,'' ``dialogue system*'') with anthropomorphism (``anthropomorph*,'' ``human-like,'' ``persona*,'' ``personality,'' ``gender,'' ``avatar''). We limited our search to peer-reviewed articles in English published between January 2018 and August 2025, capturing the modern era of CAs \cite{zhang2018personalizing}. % when neural language models became widespread.

The principal investigator (PI) screened 735 identified articles in two stages. First, the PI reviewed titles and abstracts using predefined inclusion criteria, yielding 312 potentially relevant articles. Second, they reviewed full texts. Studies were included if they reported empirical quantitative data on user interactions with conversational agents, examining at least one anthropomorphic attribute, and measuring user outcomes. The PI excluded purely technical papers without user evaluation, qualitative-only studies, reviews, and studies of physical robots without conversational interfaces. This process resulted in 79 empirical studies forming our final corpus.

\subsection{Effect Extraction and Coding}

Five trained coders extracted effects from the 79 articles using a standardized protocol (see the online appendix \footnote{\href{https://osf.io/7rj6f/overview?view_only=35b327583fee4222b5b336465107580c}{https://osf.io/7rj6f/overview?view\_only=35b327583fee4222b5b336465107580c}} for details), with 10\% of the articles were double-coded to ensure reliability. Rather than extracting generic study characteristics, we identified and coded individual effects representing relationships between anthropomorphic attributes (as independent variables) and specific user outcomes (as dependent variables) within each study. Inter-coder reliability among five coders for effect extraction was substantial (Fleiss' $\kappa$ = 0.81).
% This granular approach treats each attribute-outcome relationship as an independent observation, recognizing that single studies often examine multiple anthropomorphic features and measure various outcomes.
...
Of the 583 extracted effects, 177 (30.4\%) contained effect size calculations, which are relatively rare in HCI reporting and are referred to as calculable effects in the remainder of the article. Of these, 8 were removed due to quality issues, leaving 169 calculable effects. The other 406 effects (69.6\%) provided directional information and significance indicators but lacked details on effect sizes. We refer to them as non-calculable effects throughout the remainder of the article. For these, we removed a total of 33 effects due to missing information, resulting in a total of 373 non-calculable effects. 
This distribution of available information, in terms of calculable and non-calculable effects, necessitated a combined approaches for calculable and non-calculable effects.

\subsection{Categorization of Anthropomorphism}

To investigate which anthropomorphic attributes affect user outcomes, we categorized the attributes implemented across studies. For this, we employed data-driven classification through hierarchical agglomerative clustering. First, we extracted 73 unique attributes 
(see Table \ref{tab:attr-dist}) 
types across the 79 studies and analyzed their co-occurrence patterns, which attributes researchers implemented together, using Jaccard similarity coefficients with Ward's linkage criterion. %This approach reveals natural groupings based on empirical implementation patterns rather than theoretical assumptions. 
Analysis of the resulting dendrogram identified two primary clusters at the optimal cut-off point: behavioral attributes that manifest through agent actions during interaction (communication style, personality traits, behavioral patterns, emotional expressions) and demographic attributes representing relatively fixed aspects of agent identity (demographic identity, roles, names, visual representations, voice characteristics). Chi-square test confirmed that behavioral and demographic attributes have different prevalence rates across studies ($\chi^2$ = 8.47, p = .004), with behavioral attributes appearing in 65.5\% of studies compared to 38.8\% for demographic attributes. Detailed clustering methodology and complete attribute listings are available in the online supplementary material.

\begin{table*}[t]
\centering
\caption{\textbf{Distribution of anthropomorphic attribute categories in 79 anthropomorphization studies.} Behavioral anthropomorphization is more frequent. \textit{Note: Percentages calculated as proportion of 121 total attribute instances across behavioral and demographic categories. Studies could implement multiple attributes, so percentages do not sum to 100\%.}}
\label{tab:attr-dist}
...
& Behavioral patterns & 19 & 15.7 & proactive (n = 2), initiative-taking (n = 2), engaging \\
\midrule
...
\begin{table}[h]
\caption{Categories of dependent variables.}
...
% \subsection{Categorization of Anthropomorphic Attributes}

% Understanding which anthropomorphic attributes affect user outcomes required systematic categorization of the diverse features implemented across studies. Rather than imposing theoretical categories from prior literature, we employed data-driven classification to identify natural groupings based on how attributes were actually implemented and combined in empirical research.

% We began by extracting all anthropomorphic features examined across the 79 studies, identifying 73 unique attribute types ranging from specific personality traits and communication styles to demographic characteristics and visual representations. To understand the underlying structure of these attributes, we analyzed their co-occurrence patterns, which attributes researchers tended to implement together in their experimental designs. This co-implementation analysis reveals conceptual relationships, as researchers likely combine attributes they consider functionally related.

% We represented each attribute as a binary vector indicating its presence or absence across all studies, then applied hierarchical agglomerative clustering using Jaccard similarity coefficients to measure overlap between attribute implementation patterns. The Jaccard coefficient proved appropriate for our binary co-occurrence data, calculating similarity as the ratio of studies implementing both attributes to studies implementing either attribute. We used Ward's linkage criterion to minimize within-cluster variance, building a hierarchical structure that revealed which attributes naturally group together based on empirical implementation patterns.

% Analysis of the resulting dendrogram identified two primary clusters at the optimal cut-point, determined by examining the largest increase in fusion distance. These clusters exhibited distinct conceptual coherence. The first cluster comprised attributes that manifest dynamically through agent behavior during interaction—what we termed behavioral attributes. This cluster included communication style variations such as conversational tone, formality levels, and politeness strategies; personality trait expressions following frameworks like the Big Five; behavioral patterns including proactivity, responsiveness, and turn-taking dynamics; and emotional expressions during conversation. These attributes share the characteristic that they emerge through what the agent does and how it responds during interaction, requiring continuous behavioral manifestation rather than static assignment.

% The second cluster comprised attributes representing relatively fixed aspects of agent identity—what we termed representational attributes. This cluster included demographic characteristics such as assigned age, gender, and ethnicity; social roles or occupations; human names; visual representations through avatars or profile pictures; and voice characteristics including pitch and accent. These attributes establish who or what the agent represents rather than how it behaves, typically remaining stable throughout an interaction once initially assigned.

% Statistical analysis confirmed this behavioral-representational distinction. Across the 79 studies, behavioral features and representational features showed significantly different implementation patterns ($\chi$² = 8.47, p = .004), validating the clustering solution. Within behavioral attributes, communication style appeared in 50.0\% of studies, personality traits in 48.3\%, and behavioral patterns in 32.8\%. Within representational attributes, demographic characteristics appeared in 37.9\%, role assignments in 24.1\%, and visual appearance in 15.5\%. This prevalence data indicates that researchers more frequently manipulate behavioral features than representational features, perhaps reflecting greater accessibility of behavioral implementations or theoretical interest in dynamic interaction qualities.

% The empirically-derived behavioral-representational distinction proved essential for addressing our research question about which anthropomorphic attributes affect user outcomes. By analyzing effects separately for these two attribute categories, we could determine whether dynamic behavioral features and static representational features produce different magnitudes or patterns of effects. This categorization emerged from implementation patterns in the literature rather than theoretical imposition, grounding our analysis in how the research community actually operationalizes anthropomorphism in conversational agent design.

% \subsection{Categorization of Outcome Variables}

% The second categorization challenge involved organizing the diverse outcome measures examined across studies. The 583 effects represented relationships with numerous dependent variables, from subjective satisfaction ratings and trust measures to objective performance metrics and behavioral logs. To identify meaningful patterns in these outcomes, two coders independently performed iterative thematic analysis, grouping measures based on their underlying constructs and theoretical relationships.

% This process revealed seven outcome themes, with coders achieving 94\% agreement on effect assignments. User experience and satisfaction emerged as the most frequently examined outcome, encompassing measures of overall satisfaction, perceived usability, enjoyment, and interaction quality assessed through validated instruments. Trust and credibility formed a distinct theme capturing trust formation, perceived reliability, and willingness to rely on agent information. Learning and performance outcomes included objective measures like task accuracy, knowledge acquisition, and problem-solving success. Social perception and anthropomorphism measures assessed perceived humanness, social presence, and relationship quality. Behavioral and interaction patterns captured engagement duration, message frequency, and compliance with recommendations. Technology acceptance and adoption intentions reflected behavioral intentions to use and adoption decisions. Finally, individual differences as moderators included effects examining how user characteristics moderate anthropomorphic attribute impacts.

% These seven themes provided structure for analyzing whether anthropomorphic attributes produce consistent effects across all outcome types or whether certain attributes particularly influence specific outcomes. This categorization directly supports answering our research question by enabling outcome-specific effect size estimation and pattern identification.

\subsection{Effect Size and Meta-Analytic Procedures}

To synthesize effects quantitatively, we converted diverse statistical metrics into a common metric, Hedges' g with small-sample correction. The Hedges' $g$ effects size follows the interpretation used in Cohen's \cite{cohen2013statistical} guidelines, classifying the absolute values $\leq0.2$ as small, $>0.2$ to $\leq0.5$ as medium, $>0.5$ to $\leq0.8$ as large, and $>0.8$ as very large effects. The 177 effects with calculable statistics reported various formats: eta-squared (34.5\%), Pearson's r (16.9\%), Cohen's $d$ (15.3\%), F-statistics (9.0\%), Spearman's $\rho$ (6.8\%), chi-square (6.8\%), t-statistics (4.0\%), odds ratios (2.3\%), and other metrics (4.4\%). We applied established conversion formulas, carefully preserving effect directions. After data quality checks, removing effects with impossible values or insufficient sample sizes, 169 effects from 23 papers ($N$ = 21,104 participants) were included in the meta-analysis.
...
We assessed heterogeneity using Q-statistics and $I^2$ statistics, which quantify the percentage of variation attributable to true heterogeneity, and 95\% prediction intervals that indicate expected effect ranges. % in future studies.
...
% The combined analyses allowed us to use a larger majority of the effects, far exceeding typical meta-analyses that exclude non-calculable effects and strengthening the evidence base of which anthropomorphic attributes affect which user outcomes.

% Our study directly targets the research question through multiple complementary analyses. First, overall meta-analysis establishes whether anthropomorphic attributes generally affect user outcomes and estimates the average magnitude. Second, attribute-type comparisons (behavioral versus demographic) reveal whether dynamic interaction qualities and static identity features differ in their impacts, thereby informing whether designers should prioritize agent behavior or representation. Third, outcome category analyses identify where anthropomorphic attributes matter most---whether effects are concentrated in the experiential, social, performance, or behavioral domains. Fourth, triangulation between quantitative and qualitative synthesis methods tests the robustness of the evidence across evidence types. Finally, analysis of implementation patterns from the broader 79-article corpus contextualizes the findings by documenting which attributes researchers actually implement and how frequently, revealing alignment or misalignment between research attention and empirical effects.

\section{Results}

Our research question asks how anthropomorphic attributes of CAs affect user outcomes. We present findings progressing from overall effects to attribute-specific patterns, revealing which design features produce strongest outcomes through quantitative meta-analysis of 169 effects, qualitative synthesis of 365 additional effects, and triangulation across methods.

\subsection{Overall Effects}

The random-effects meta-analysis revealed that anthropomorphic attributes produce moderate positive effects on user outcomes, $g$ = 0.73, 95\% CI [0.59, 0.87], $p$ < .001. Users interacting with anthropomorphic CAs achieve outcomes approximately 0.75 standard deviations better than those with non-anthropomorphic agents. However, substantial heterogeneity, $I^2$ = 95.4\%, indicates considerable variation across studies 
% (see Figure \ref{fig:forestplot}). 
We employed the DerSimonian-Laird random-effects model \citep{dersimonian1986meta} to test for heterogeneity. The estimated variance between studies was $\tau^2 = 0.75 (\tau = 0.86)$, indicating a substantial heterogeneity in the effect sizes between studies. The 95\% prediction interval $[-0.97, 2.43]$ reveals that individual implementations range from small negative to large positive effects, suggesting a heterogeneity in prediction ranging from negative to positive effects. This heterogeneity motivated our analyses of specific design attributes and outcomes. The 95\% prediction interval indicates that individual implementations range from small negative to large positive effects, though observed studies show predominantly positive outcomes (84.2\% positive, see Section \ref{}). 
...
We acknowledge that including multiple effects from the same study (M = 7.3 effects per article) violates independence assumptions. To assess this impact, we conducted sensitivity analysis aggregating effects to one per study, yielding $g$ = 0.54, 95\% CI [0.46, 0.62] compared to $g$ = 0.73, 95\% CI [0.59, 0.87] in the full model, which is a 26\% reduction. Despite this difference, we report the full 169-effect model as primary for three reasons. First, our research question examines which attributes affect which outcomes, requiring effect-level granularity that article-level aggregation cannot provide. Second, both estimates show moderate positive effects with overlapping confidence intervals, indicating a robust finding. Third, modern meta-analytic guidance \cite{} supports retaining effect-level data when dependency can be modeled using robust variance estimation or multilevel approaches. 
...
Anthropomorphic attributes affected outcomes differently depending on what was measured. Analysis across seven outcome categories revealed systematic variation ($Q_{\text{between}}$(6) = 48.23, $p$ < .001), indicating that different attributes have different effect sizes.

UX outcomes showed large effects ($g$ = 0.69, 95\% CI [0.50, 0.88], $k$ = 73). As the most frequently examined category, this represents a well-established pattern. Perceived usefulness showed moderate to large effects ($g$ = 0.84, 95\% CI [0.28, 1.39], $k$ = 11), while perceived ease of use showed larger effects ($g$ = 1.34, 95\% CI [0.42, 2.26], $k$ = 7). For designers prioritizing user satisfaction and usability, anthropomorphic features offer meaningful improvements across multiple experiential dimensions.

Social perception outcomes, measuring perceived humanness, social presence, and relationship quality, showed moderate effects ($g$ = 0.430, 95\% CI [0.143, 0.716], $k$ = 24). Social presence specifically showed the largest effect in the dataset ($g$ = 2.04, 95\% CI [1.00, 3.08], $k$ = 4). These findings indicate that anthropomorphic design most directly influences how human-like and socially present agents appear to users, which represents the proximal goal of anthropomorphic implementation.

Trust and credibility outcomes demonstrated moderate-to-large effects ($g$ = 0.66, 95\% CI [0.45, 0.87], $k$ = 24). Trust specifically produced a moderate effect ($g$ = 0.59, 95\% CI [0.20, 0.98], $k$ = 11), while reliability perceptions showed larger effect sizes ($g$ = 0.93, 95\% CI [0.57, 1.29], $k$ = 4). Anthropomorphic design appears to address a key challenge in CAs, which is establishing user confidence in the system's credibility.

Learning and performance outcomes showed moderate effects ($g$ = 0.75, 95\% CI [0.16, 1.34], $k$ = 11), with achieved performance demonstrating a relatively larger effect ($g$ = 1.00, $k$ = 3). These results suggest that anthropomorphism extends beyond subjective perceptions to influence objective task outcomes, indicating value for educational and task-support applications.

Effects were smaller for more diverse outcomes. Technology acceptance showed small to moderate effects ($g$ = 0.71, 95\% CI [0.17, 1.246], $k$ = 5), while behavioral and interaction patterns showed moderate effects ($g$ = 0.67, 95\% CI [0.31, 1.03], $k$ = 13). This pattern suggests that while anthropomorphism shapes user outcomes, it influences actual usage behaviors more modestly, likely because adoption decisions depend on multiple factors beyond agent design.
Finally, individual differences as moderators showed small effects ($g$ = 1.137, 95\% CI [0.65, 1.623], $k$ = 19), indicating that the effects of anthropomorphism on user outcomes appear relative consistent between different user populations. %while user characteristics modestly influence anthropomorphic effectiveness, the benefits appear relatively consistent between user populations.

\begin{figure}
    \centering
    \includegraphics[width=0.65\linewidth]{Figures/figure3_outcome_categories.pdf}
...
While behavioral attributes generally outperform demographic attributes, effect sizes vary substantially depending on the specific outcome measured. We analyzed which anthropomorphic attributes produce the strongest effects for which user outcomes (see Figure \ref{fig:ivs_dvs}), presented as follows. %, revealing systematic patterns that inform targeted design decisions.

\subsubsection{Communication style improves UX}\label{sec:comm}

Communication style attributes indicated strong effectiveness for UX outcomes ($g$ = 1.89, $k$ = 13), nearly double the effect observed for other attribute types. Features like politeness calibration, formality adaptation, and natural phrasing produce large effects when measuring satisfaction, usability, and enjoyment. This pattern held with 76.9\% of effects reaching significance, indicating consistent benefits.

For social perception outcomes, communication style again showed large effects ($g$ = 1.39, $k$ = 6, 83.3\% significant), though with greater variability. Users perceive agents employing sophisticated communication strategies as more human-like and socially present. However, communication style showed more modest effects on behavioral patterns ($g$ = 1.32, $k$ = 3), suggesting that while users appreciate natural language, it moderately influences actual interaction behaviors, like message frequency or session duration.

\subsubsection{Visual Appearance Shows Outcome-Specific Patterns}

Visual appearance attributes displayed striking outcome dependence. For trust and credibility, visual features produced moderate-to-large effects ($g$ = 0.70, $k$ = 5), suggesting that professional avatars or appropriate visual design meaningfully influence perceived trustworthiness. However, for UX outcomes, identical visual attributes showed much smaller effects ($g$ = 0.35, $k$ = 12, only 25\% significant), indicating that visual design alone rarely enhances satisfaction or usability.
Unexpectedly, visual appearance showed very large effects when examined as a moderator of other relationships ($g$ = 3.14, $k$ = 3). This suggests that visual features may amplify or attenuate the effects of other attributes rather than producing strong direct effects themselves, though this finding requires cautious interpretation given the small sample.

\subsubsection{Demographic identity improves learning outcomes}

Demographic attributes—assigned age, gender, ethnicity, or cultural identity—produced their strongest effects for learning and performance outcomes ($g$ = 1.00, $k$ = 3, 66.7\% significant). When agents had relevant demographic characteristics, users showed better task performance and knowledge acquisition, which suggests that demographic matching or appropriate identity assignment matters most when the agent serves educational or performance-support functions.
For UX, demographic attributes showed moderate effects ($g$ = 0.68, $k$ = 8), but only 37.5\% reached significance, indicating inconsistent benefits. Users sometimes appreciate demographic cues but not reliably enough to justify prioritizing them in general-purpose conversational interfaces.

\subsubsection{Behavioral patterns work across contexts}

Behavioral pattern attributes---including proactivity, repair strategies, and interaction mechanisms---showed consistent moderate effects across diverse outcomes. For learning ($g$ = 0.87, $k$ = 3, 100\% significant), trust ($g$ = 0.20, $k$ = 11, 100\% significant), and behavioral patterns themselves ($g$ = 0.41, $k$ = 7, 100\% significant), these features reliably enhanced outcomes. While effect magnitudes remained moderate rather than large, the perfect success rate across trust outcomes indicates that behavioral features, like appropriate repair strategies, consistently improve perceived reliability.

\subsubsection{Personality traits show dual effectiveness}\label{sec:person}

Personality trait implementations produced large effects for UX ($g$ = 0.65, $k$ = 10), though with a modest 40\% significance rate, suggesting high variability, %. 
%When personality coherence succeeds, effects are substantial; when poorly implemented, benefits disappear. 
which may reflect implementation differences, so that consistent personality expression across interactions produces strong effects, while superficial or inconsistent trait displays fail.
...
\subsection{Triangulation from Multiple Syntheses}

Three complementary synthesis methods validated the quantitative findings using the 373 non-calculable effects. Vote counting revealed that among directional effects, 84.2\% were positive--exceeding chance expectation, $p$ < .001 (binomial test). Fisher's combined probability test across 269 effects indicated that the pattern of statistical significance differs from chance, $\chi^2$(538 (2$\times$269) = 1115.85, $p$ < .001. Stouffer's Z-score method indicated predominantly positive effects, $Z$ = 8.84, $p$ < .001. These values (0.025 for significant, 0.50 for non-significant) represent midpoints between typical thresholds and extremes; sensitivity analyses using alternative substitutions (0.01–0.03 for significant, 0.40–0.60 for non-significant) yielded identical conclusions (all $p$ < .001).

All three methods detected significant effects across all seven outcome categories (all $p$-values < .01) and for both behavioral and demographic attribute types, with behavioral attributes consistently showing stronger evidence. This convergence strengthens confidence that findings represent robust patterns rather than artifacts of any single analytical approach.

Implementation pattern analysis across the 79-article corpus revealed that research practice aligns with effectiveness findings. Behavioral features dominate design choices: communication style appeared in 50.0\% of studies and personality traits in 48.3\%, compared to demographic attributes (37.9\%), assigned roles (24.1\%), and visual appearance (15.5\%). Overall, behavioral features appeared significantly more frequently than demographic features (77.6\% versus 41.4\%), $z$ = 3.97, $p$ < .001, Cohen's $h$ = 0.76. Cohen's $h$ was used to measure the effect size of the difference between two proportions. 
...
Publication bias assessment revealed mixed findings. Egger's regression test detected significant asymmetry (intercept = 2.21, $SE$ = 0.71, $t$(167) = 3.11, $p$ = .002), indicating smaller studies reported larger effects. However, fail-safe $N$ analysis showed 620 unpublished null studies would be needed to nullify results, exceeding Rosenthal's criterion \cite{rosenthal1979file} of 125 ($5\times23+10$) by a factor of 4.96, suggesting robustness despite potential small-study bias.

Our synthesis of 583 effects from 79 studies indicates that anthropomorphic attributes consistently produce positive effects, $g$ = 0.73, with effectiveness depending critically on implementation choices. Behavioral attributes, particularly communication style ($g$ = 0.95), personality traits ($g$ = 0.78), and responsiveness ($g$ = 0.69), most consistently improve outcomes, outperforming demographic attributes ($g$ = 0.59). Effects concentrate most strongly on social perception ($g$ = 1.20) and UX ($g$ = 0.78), with moderate effects for trust ($g$ = 0.62) and learning ($g$ = 0.59), and smaller effects for behavioral outcomes ($g$ = 0.39). All synthesis methods are consistent with these conclusions. %, strengthening confidence in the findings.
...
Our findings are consistent with CASA theory \citep{nass1997machines}, which postulates that social cues in computer interfaces trigger social responses. Both behavioral and demographic anthropomorphic attributes produced positive effects with behavioral attributes (communication style, personality traits, responsiveness) ($g$ = 0.75), while demographic attributes (names, demographic identity, visual appearance) yielded ($g$ = 0.69). These effect sizes indicate that both attribute types substantially improve user outcomes.
Within these categories, we observed notable variation. Communication style showed the strongest effects, followed by personality traits. Among demographic attributes, visual appearance  showed stronger effects than demographic identity assignments. These differences suggest that specific implementation choices matter considerably for anthropomorphic effectiveness.
Our contribution here is quantifying these effects and identifying which specific anthropomorphic features produce the strongest outcomes. Communication style and personality expression emerge as particularly effective design choices. For practitioners, this suggests prioritizing how conversational agents communicate---their tone, word choice, and personality expression---over demographic assignments or labels. While CASA theory predicted that anthropomorphic cues influence users, our meta-analysis specifies which cues warrant design attention. This indicates a shift in design emphasis from ``what the agent appears to be'' to ``how the agent continuously acts.''

Our results also complement and extend Klein's \cite{klein_effects_2025} meta-analysis of text-based conversational agents ($g$ = 0.36, 142 studies), with three key differences. First, our overall dependent effect ($g$ = 0.73) or even independent effect ($g$ = 0.54) is substantially larger, likely because we include voice-based and multimodal agents in which behavioral attributes like tone, prosody, and timing amplify anthropomorphism beyond text alone. Second, our behavioral-demographic division provides theoretical specificity that Klein's broad ``human-like social cues'' does not. We show that within anthropomorphic design, behavioral implementation choices matter more than demographic ones. Third, Klein's text-only focus could not examine attributes like voice feedback style ($g$ = 1.92) or information modality ($g$ = 1.44) that our findings reveal as highly effective. Overall, our findings corroborate the idea that anthropomorphism improves user outcomes \citep{klein_effects_2025} and extend this idea to apply across modalities, with behavioral features predominantly driving the effects.

%\textit{Practical Implications: }
Based on our findings, we propose six design guidelines (DGs):

\textbf{DG1: Prioritize communication style over visual design} (Section \ref{sec:beh}). Invest development resources in natural language generation, formality adaptation, and politeness strategies rather than avatar selection. Implement context-appropriate formality (formal for professional tasks, informal for casual interaction) and consistent politeness calibration.

\textbf{DG2: Implement coherent personality} (Section \ref{sec:person}). CAs should express consistent personality across all interactions. If designing an agreeable agent, nearly every response should emphasize cooperation. When implementing extraversion, the CA can initiate conversations and convey enthusiasm throughout, which can help avoid personality inconsistencies that break the social illusion.

\textbf{DG3: Build proactive responsiveness} (Section \ref{sec:beh}). CA developers should implement adaptive behaviors, including proactive dialogue actions, intelligent turn-taking, and effective repair strategies. Ideally, the CA anticipates user needs without seeming intrusive, and responds appropriately when misunderstandings occur (suggestion versus rephrase strategies).

\textbf{DG4: Match anthropomorphic attributes with outcomes} (Section \ref{sec:out}). For social applications (e.g., companions, tutors), CA developers should prioritize features that improve social perception. For user satisfaction goals (e.g., customer service, information systems), it is better to focus on UX. For trust-critical applications (e.g., health, finance), emphasizing behavioral authenticity is vital.

\textbf{DG5: Anthropomorphize behavior, not demographics} (Section \ref{sec:beh}). CA developers should tailor the CA's communication style to user preferences (formal versus informal, verbose versus concise) and personality expression to user traits rather than matching demographic characteristics. Behavioral personalization outperforms demographic-based personalization.

\textbf{DG6: Manage expectations for behavioral outcomes} (Section \ref{sec:out}). While anthropomorphism substantially improves perceptions and experiences, its effects on adoption intentions and actual usage are modest. Anthropomorphism complements but does not substitute functional quality and task effectiveness in technological systems. Therefore, it is vital to acknowledge the limits of anthropomorphism alongside other factors influencing user outcomes.

%\subsection{Limitations and Future Directions}
%\textit{Limitations and Future Directions: }
When interpreting our findings, some limitations should be considered. First, multiple dependent effects per study (M = 7.3) likely underestimate standard errors. Sensitivity analysis aggregating to one effect per study yielded consistent but smaller effects ($g$ = 0.54 vs. 0.73), suggesting the primary estimate may be somewhat inflated. Future meta-analyses should employ multilevel models \cite{vandennoortgate2013} or robust variance estimation \cite{hedges2010}. Second, Egger's test detected significant funnel plot asymmetry ($p$ = .002), indicating small studies reported larger effects. This could reflect publication bias or genuine methodological differences between small and large studies. The large fail-safe $N$ (n = 620) suggests that findings remain robust despite this asymmetry, although effect magnitude estimates should be interpreted cautiously. Third, substantial heterogeneity ($I^2$ = 95.4\%) and a wide prediction interval [-0.97, 2.43] indicate considerable variation across studies, with some CA implementations producing negative effects. 
...
% \subsubsection{Feature-Specific Evaluations}

% When users evaluated specific features within conversational systems, differential preferences emerged. Among COVID-19 chatbot features, FAQs received the highest ratings ($M = 4.83$, $SD = 0.45$), followed by Cases Tracker ($M = 4.72$, $SD = 0.51$), WHO Advice ($M = 4.61$, $SD = 0.60$), and Screening/Symptoms Checker ($M = 4.56$, $SD = 0.77$) \cite{A79}. Despite the symptoms checker receiving the lowest rating, all features scored highly (above 4.5 on a 5-point scale), leading to the recommendation that chatbots should incorporate multiple feature types rather than focusing solely on symptom checking functionality \cite{A79}.
...
% Interface modality alone does not significantly affect trust perceptions \cite{A15}, indicating that trust must be built through content, behavior, and interaction quality rather than channel characteristics. Domain-specific applications face particular challenges in perceived question understanding, which emerged as the weakest trust dimension for professional training chatbots \cite{A54}.
...
% This finding supports personalization approaches that allow users to adapt conversational agent personality to their preferences in real-time. Rather than forcing a single interaction style, providing character options acknowledges individual differences in communication preferences and enables users to optimize their experience dynamically. The high rating for character switching validates character-based design frameworks that offer choice rather than imposing uniform personality presentation.
...
% The finding highlights that personal space perception is contextual rather than purely spatial. The same physical distances feel different depending on agent behavior patterns, movement dynamics, and user control over the interaction. Companion designs that maintain constant proximity may trigger greater personal space awareness than guide designs that establish distance through leading behavior.
...
% This finding indicates that anthropomorphic design features may differentially appeal to user segments. Users who approach decisions emotionally and holistically may be more receptive to chatbots presenting human-like social cues, while users who approach decisions analytically may be less influenced by---or even prefer to avoid---anthropomorphic presentation. Personalization of agent human-likeness based on assessed user characteristics represents a potential optimization strategy.

% \subsubsection{Agent Personality and User Experience}

% The AI-driven VTuber KawAIi demonstrated that agent personality characteristics contribute to positive user perceptions. The system featured a polite, friendly character designed to deliver entertaining interaction through emotion-aware expressions and engaging communication style \cite{A81}.
...
% Individual differences moderate preferences for human-likeness, with intuitive decision-makers showing greater receptivity to anthropomorphic features than rational decision-makers \cite{A42}. This finding supports personalization of agent presentation based on user characteristics.
...
% An interesting interaction emerged between dialogue-initiation preferences and starting conditions: higher user-initiative preference was negatively correlated with initiative transfer in user-initiated starting mode ($r = -0.299$) \cite{A42}. This counterintuitive finding suggests that users with strong preferences for controlling dialogue, when given initial control, maintain that control throughout the interaction rather than ceding and then reclaiming initiative. The initiative transfer measure may primarily capture recovery of control after system initiation rather than general initiative-taking tendencies.
...
% The finding has implications for social design of conversational agents: photorealistic embodiment may paradoxically reduce social courtesy if users perceive the representation as a recording rather than a responsive entity.
...
% \subsubsection{Feature-Specific Adoption Value}

% User evaluations of specific features within conversational systems provide insight into which capabilities drive adoption value. The COVID-19 chatbot Chasey's features all received high ratings, but with differential enthusiasm \cite{A79}.

% FAQs achieved the highest rating ($M = 4.83$, $SD = 0.45$), followed by Cases Tracker ($M = 4.72$, $SD = 0.51$), WHO Advice ($M = 4.61$, $SD = 0.60$), and Screening/Symptoms Checker ($M = 4.56$, $SD = 0.77$) \cite{A79}. Despite the symptoms checker receiving the lowest rating, all features exceeded 4.5 on a 5-point scale, indicating strong perceived value across the feature set.

% The recommendation against creating chatbots solely for symptom checking---instead incorporating multiple feature types---reflects adoption considerations \cite{A79}. A multi-feature chatbot provides value across diverse user needs and use occasions, potentially supporting more frequent and sustained adoption than single-purpose designs. Users who find one feature less relevant may still adopt the chatbot for other capabilities.

% The lower standard deviation for FAQs ($SD = 0.45$) compared to symptoms checking ($SD = 0.77$) indicates greater consensus about FAQ value, while symptoms checking opinions varied more widely. This pattern suggests that informational features achieve more universal adoption appeal than interactive diagnostic features, possibly due to variable user confidence in self-administered health assessments.
...
% User preferences for controlling conversational initiative moderated actual initiative-taking behaviors. User-initiative preference---desire to direct conversations rather than follow system lead---predicted greater likelihood of taking initiative back from system-initiated dialogues ($\beta = 0.546$) \cite{A42}. Conversely, system-initiative preference---comfort with system-directed interactions---predicted lower likelihood of reclaiming initiative ($\beta = -0.559$) \cite{A42}.
...
% An interaction between initiative preference and dialogue starting condition emerged: higher user-initiative preference negatively correlated with initiative transfer in user-initiated starting mode ($r = -0.299$) \cite{A42}. Users who strongly prefer to control conversations, when given initial control, maintain that control consistently rather than transferring initiative. This pattern suggests that initiative preferences manifest differently depending on who begins the conversation.
...
% This finding aligns with self-determination theory predictions that autonomy support enhances motivation and engagement while autonomy-threatening interventions provoke reactance. Conversational interfaces should position themselves as helpful informants and suggesters rather than autonomous actors that take over user tasks. Even when intervention might produce objectively better outcomes, the trust and experience costs may outweigh instrumental benefits.
...
% \textbf{Prioritize conversational competence.} Natural language understanding and response generation quality represent foundational requirements that cannot be compensated by other design features. Ensure that the system can understand user input and provide accurate, relevant responses before refining personality, embodiment, or interface modality.

% \textbf{Adopt the notification principle.} Design proactive behaviors that inform, suggest, and offer rather than intervene, override, or mandate. Preserve user autonomy even when system capabilities might enable autonomous action. Users trust and accept systems that enhance their agency rather than supplant it.
...
% \textbf{Social presence mechanisms.} The gap between functional competence and social believability invites research on what specific design features contribute to social presence. Whether improvements in behavioral naturalness, conversational depth, or emotional responsiveness can close this gap remains to be determined.
...
% Cross-cutting patterns revealed that conversational competence serves as a foundational requirement that other design features cannot compensate for; notification-based proactive behaviors consistently outperform autonomy-threatening interventions; likability and trust operate through partially independent mechanisms; interface modality represents a secondary consideration with limited impact on measured outcomes; personalization and flexibility enhance adoption appeal; functional competence does not automatically confer social believability; and educational applications represent particularly suitable deployment contexts.
...
% \midrule
% \textit{Typical features} & Conversational tone, empathy, proactive responses, politeness markers & Human name, gender, professional title, avatar embodiment \\
% \midrule
...
% % While multimodal interfaces are emerging, the majority of CAs (51.7\%) remain text-only. For practitioners, this indicates that establishing a social actor does not strictly require high-cost investments in 3D avatars or voice synthesis. The frequency of ``Communication Style'' as a dominant attribute indicates that anthropomorphism is often achieved through script design (e.g., tone, politeness) rather than visual embodiment.

% % While fixed personas remain the norm (72.4\%), adaptive anthropomorphism was the only implementation feature to show a significant increase over time ($\rho = .83$). This trend indicates that the ``state of the art'' is moving toward systems that adjust their personality traits dynamically based on user input. Practitioners looking to align with the leading edge of research might consider moving beyond static profiles toward context-aware adaptation, i.e., the anthropomorphic attributes of the CA could be divided into two groups. One is the static and consistent attributes that remain fixed, while the second group should be adaptable, given the context.
...
% % First, research has moved away from assigning human identities toward designing human-like behavior. Most studies examined agents' communication through tone, style, and personality, rather than the human identity they appear to represent. Communication style and personality traits appeared far more frequently than demographic or visual features. This shift suggests, perhaps, that users respond more to behavioral interaction patterns than to demographic traits, or at least CA designers think they do. Earlier work on the CASA framework has shown that people react socially to minimal cues \citep{gambino2020building}. Current research applies this principle by implementing politeness, empathy, and natural dialogue rather than attempting to disguise artificiality. In some studies, systems openly acknowledged their artificial nature yet still achieved some social presence. This suggests that engagement depends on the quality of communication, rather than deception. Designers can therefore maintain both transparency and social effectiveness.

% % Second, anthropomorphic attributes tend to appear in connected groups (clusters) rather than as isolated elements. When CA designers assigned a human name, they often also included gender cues; when an avatar was present, a voice was usually included. These combinations show that one human-like feature often triggers expectations for others. When features are aligned, for instance, empathetic language paired with supportive tone, the anthropomorphic traits of CAs feel coherent and believable. However, when cues conflict, for example, formal messages combined with playful emojis, CAs are likely to appear inconsistent. Effective anthropomorphic attributes construction, therefore, depends on internal alignment where visual, verbal, and behavioral traits mutually reinforce each other. Researchers have identified two broad clusters of attributes: a behavioral cluster focused on communication and personality, and an identity cluster focused on demographic and visual traits. CAs generally combine features within one cluster rather than mixing them.
...
% % \textit{First}, anthropomorphic choices must match the purpose of the system. For example, in healthcare, ethical transparency requires clear disclosure that the CA is artificial and has a personality, while empathetic communication and professional authority support patient trust. Similarly, in education, CAs could benefit from striking a balance between engagement and credible expertise. In less complex situations, such as customer service, the CAs' tone should reflect the organization’s brand identity and promote efficiency. Each context, therefore, requires its own selection of anthropomorphic attributes rather than applying a single model of anthropomorphism.

% % \textit{Second}, coherence is essential. Adding isolated human-like features—such as a friendly voice or avatar—does not guarantee positive results if other cues conflict. Visual, verbal, and behavioral traits should form one unified character. For example, a professional-sounding voice should be matched by precise, consistent messages, not casual phrasing. Adaptive systems should define in advance which features can change and which remain stable. This preserves continuity and prevents users from perceiving behavioral inconsistency.

% % \textit{Third}, evaluation and monitoring must rely on real usage rather than technical measures alone. Designers should assess not only satisfaction but also user behavior, such as task completion and long-term engagement. Evaluations should involve diverse user groups because preferences vary by age, expertise, and cultural background. For instance, speech patterns considered polite in one culture may appear distant or insincere in another. Longitudinal evaluation helps detect gradual shifts in trust and expectations that short-term tests can miss.

% % \textit{Fourth}, several common pitfalls should be avoided. Some systems create unrealistic expectations by presenting themselves as emotionally capable when their responses remain scripted, creating the uncanny valley effect. Others could reinforce stereotypes through gender and occupation pairing, such as giving female identities to service roles or male identities to technical ones. Demographic cues should only be used when relevant to function. Another frequent oversight is deploying a single static persona even when users differ widely in communication preferences. Finally, optimizing for technical fluency alone often hides usability problems; designers should measure perceived usefulness and trust rather than linguistic accuracy alone.

% \subsection{Limitations and Future Directions}

% This systematic review is subject to specific limitations that shape the interpretation of our findings. First, the analysis is constrained by the reporting transparency of the primary studies. particularly in recent LLM research, authors often describe a persona qualitatively (e.g., ``empathetic assistant'') without disclosing the specific system prompts or parameter settings used to generate it. Consequently, our analysis is based on the attributes described by authors rather than the underlying technical ground truth, potentially missing implicit anthropomorphic attributes that are ``hidden in the code'' or otherwise opaque. 

% Second, the review covers a period of extreme technological evolution from 2018 to 2025. Grouping early rule-based CAs with advanced generative models under a single category is necessary for a longitudinal view, but it may obscure how user definitions of ``intelligence'' and ``consistency'' have shifted as the underlying technology evolved. Third, this study maps prevalence rather than efficacy. We provide descriptive evidence of what the research community considers important, rather than evidence on the efficacy (i.e., the impact of anthropomorphic attributes).
...
This meta-analysis of 583 effects from 79 studies demonstrates that anthropomorphic attributes consistently improve user outcomes; however, their effectiveness depends on the implementation choices made. Behavioral attributes, communication style, personality, and responsiveness, outperform demographic attributes like demographic identity or visual appearance. Effects concentrate on social perception and UX, with moderate effects on trust and learning. These findings extend the CASA theory and complement previous meta-analyses on CAs, providing clear guidance on how agents communicate and behave, rather than focusing on their appearance or representation.
